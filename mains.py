...
def RegisterPage():
    ...
    global securityanswer
    canvas.delete('all')
    backgroundIMG=PhotoImage(file="backg.gif")
...
def feedpage():
    ...
    global feedEntry
    canvas.delete('all')
    gifimg=PhotoImage(file="backg.gif")
    canvas.create_image(0,0,image=jpgimg,anchor=NW)
...
def viewbooking():
    ...
    rebutton=Button(canvas,text="booking page",width=15,fg="white",bg="Red",bd=0,font=("Arial",13,'bold'),command=gotocpage)
    canvas.create_window(1200,50,window=rebutton)
    backgroundIMG=PhotoImage(file="backg.jpg")
    canvas.create_image(0,0,image=backgroundIMG,anchor=NW)
    x,y=0,30
